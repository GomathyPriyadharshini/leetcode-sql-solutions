...
-- revenue (quantity × price). If there is still a tie, return the
...
with cte as (select case when Month(sale_date) in(12,1,2) then 'Winter'
when Month(sale_date) in (3,4,5) then 'Spring'
when Month(sale_date) in (6,7,8) then 'Summer'
when Month(sale_date) in (9,10,11) then 'Fall'
end as season,category,sum(quantity) as total_quantity,
...
(select *,rank() over(partition by season order by total_quantity desc,total_revenue desc,category) as rnk from cte)t2
where rnk=1
order by season
