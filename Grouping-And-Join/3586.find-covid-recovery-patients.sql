--
-- @lc app=leetcode id=3586 lang=mysql
--
...
-- 
-- 
-- Return the result table ordered by recovery_time in ascending order, then by
...
with povtbl as (select patient_id,min(test_date) as test_date from covid_tests where result='Positive' group by patient_id),
negtbl as (select patient_id,min(test_date) as test_date from covid_tests t1 where result='Negative'
and exists(select 1 from povtbl t2 where t1.patient_id=t2.patient_id and t1.test_date>t2.test_date)
group by patient_id)

select povtbl.patient_id,patient_name,age,
datediff(negtbl.test_date,povtbl.test_date) as recovery_time
from povtbl povtbl inner join negtbl negtbl
on povtbl.patient_id=negtbl.patient_id
and povtbl.test_date<negtbl.test_date
inner join patients p
on povtbl.patient_id=p.patient_id
