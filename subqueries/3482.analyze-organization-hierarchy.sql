...
-- David (ID: 4), Eva (ID: 5) report to Bob, while Frank (ID: 6) and Grace (ID:
-- 7) report to Charlie (level 3)
...
-- Eva, Grace, Hank, Ivy, and Judy have no direct reports (team_size =
-- 0)
-- 
-- 
-- Budget Calculation:
...
-- Employees with no direct reports have budgets equal to their own
-- salary
-- 
-- 
-- 
...
(
    ...
    SELECT
        ...
        c.level + 1 AS level
    FROM employees t
    INNER JOIN level AS c
        ON t.manager_id = c.employee_id
),
...
SELECT
    ...
    COALESCE(SUM(emp.salary), 0) + e.salary AS budget
