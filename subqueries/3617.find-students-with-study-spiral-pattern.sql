...
-- 
-- 
-- A Study Spiral Pattern means a student studies at least 3 different subjects
-- in a repeating sequence
-- The pattern must repeat for at least 2 complete cycles (minimum 6 study
-- sessions)
-- Sessions must be consecutive dates with no gaps longer than 2 days between
-- sessions
...
-- 
-- 
-- Return the result table ordered by cycle length in descending order, then by
-- total study hours in descending order.
-- 
...
-- 
-- 
-- Study sequence: Math → Physics → Chemistry → Math → Physics →
-- Chemistry
...
-- Pattern: 4 subjects (Algebra, Calculus, Statistics, Geometry) repeating for
-- 2 complete cycles
-- Consecutive dates: Oct 1-8 with no gaps > 2 days
-- Cycle length: 4 subjects
-- Total hours: 4.0 + 3.5 + 2.5 + 3.0 + 4.0 + 3.5 + 2.5 + 3.0 = 26.0
-- hours
-- 
-- 
...
WITH base AS (
    SELECT
        *, 
        ROW_NUMBER() OVER(PARTITION BY student_id, subject ORDER BY session_date) AS spiral_id 
    FROM study_sessions
),
...
filter_more_than_two_cycle AS (
    ...
    GROUP BY student_id, subject_group, cycle_length
    HAVING COUNT(*) >= 2
)
...
SELECT 
    ...
    f.cycle_length, 
    f.total_study_hours
FROM filter_more_than_two_cycle f
